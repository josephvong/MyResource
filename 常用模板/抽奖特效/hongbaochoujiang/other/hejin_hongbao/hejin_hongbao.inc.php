...
if($model == 'angpow'){
	$aid = intval($_GET['aid']);
	if($_GET['isgz']==1){
		setcookie('hjhb_ygz', 1, time()+31536000);
		...
		if($ishave){
			...
			if($angpow['is_qiyong']==1 && $angpow['end_time']>time() && $angpow['start_time']<time()){
				...
				if($shenyuje >= $angpow['upnumber']){
					...
					if(!$_COOKIE['hjhb_ygz']){
						header("Location: ".HEJIN_URL."&model=weixin&aid=".$aid."");
					}
					//用户是否登陆
					if($_COOKIE['hjhb_uid']>0){
						$islogin = 'true';
						$uid = intval($_COOKIE['hjhb_uid']);
...
elseif($model == 'login'){
	...
	if($_GET['formhash']==formhash()){
		if($tel){
			...
			if($uhave){
				setcookie('hjhb_uid', $user['id'], time()+31536000);
				setcookie('hjhb_telphone', $user['telphone'], time()+31536000);
				$sharees =  C::t('#hejin_hongbao#hjhb_shares')->fetch_by_auid($aid,$user['id']);
				$haveshare = count($sharees);
				if(!$haveshare){
					$ssadata=array();
					$ssadata['uid']=intval($user['id']);
					$ssadata['aid']=$aid;
					$ssadata['add_time']=time();
					$ssaaveadd = C::t('#hejin_hongbao#hjhb_shares')->insert($ssadata);
				}
				...
				if($adduser){
					...
					setcookie('hjhb_uid', $adduser, time()+31536000);
					setcookie('hjhb_telphone', $tel, time()+31536000);
				}
			}
			if($_COOKIE['hjhb_uid']>0){
				echo $_COOKIE['hjhb_uid'];
			}else{
				echo 0;
			}
		}
...
elseif($model == 'gotangpow'){
	...
	if($_COOKIE['hjhb_uid']>0){
		...
		if($uuid){
			$sharees =  C::t('#hejin_hongbao#hjhb_shares')->fetch_by_auid($aid,$uuid);
			$haveshare = count($sharees);
			if($haveshare){
				$ushareda = array();
				if($sharees['is_ok']==0){
				...
				$ushare = C::t('#hejin_hongbao#hjhb_shares')->update_by_id(intval($sharees['id']),$ushareda);
			}
		}
